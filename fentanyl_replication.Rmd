...
```{r begin table 2 original, results='asis', fig.pos="H", fig.cap="Corrected Table 2 results. The OLS Model 1 is represented by fentr and the 2SLS Model 2 by fent_r(fit).", comment=NA,cache.comments=NA, message=FALSE}
#################################### TABLE 2
...
|    Nearly all of the extension results either produced drastically larger estimates or drastically smaller estimates for Table 2 of Zoorob's analyses. Zoorob's Table 2 calculated by using the first definition of fentanyl exposure is shown below.
...
```{r ext 1 table 1, results='asis', comment=NA, fig.pos="H",eval=FALSE}
############ FENTANYL EXPOSURE 2017
...
|    Zoorob's fentanyl coefficient estimates in the corrected table for Models 1 and 2 are 4.508 and 5.443, expressing the difference in the effect of fentanyl between two state-years. The estimates from the extension are 2.380 and 5.549, 0.022 and 0.060, and 39.298 and 84.072 for each definition respectively. These coefficients on a state's fentanyl exposure changed considerably to reasonable changes in the definition of fentanyl exposure, and yield either unrealistic or impossible estimated deaths. The ordinary least squares estimated fentanyl deaths from extensions 1 and 3, total to 234,037 and 238,812 respectively, much larger than Zoorob's corrected estimates, which total to 96,969 deaths. Zoorob's corrected 2016 estimate, 26,491 deaths, is also more consistent with the official mortality statistic of 19,400 than either of the extension estimates: 46,641, 5,732, and 47,610 respectively. Estimates from extensions 1 and 3 appear to be too high, and estimates from extension 2 appear to be too low to be consistent with official mortality statistics. 

|    We might also compare the results of this extension using the R^2^ values of each regression. In this case, Zoorob's corrected table's R^2^ of 9.14 and .908 respectively suggest that the original definition and model can explain more variation in overdose mortality than the proposed extensions, which all yielded R^2^ values below, and in some cases extremely below, 0.9. Zoorob's method of defining fentanyl exposure appears to explain more of the variation in mortality rate than any of the extensions. The proposed extensions do not appear to improve the model or predict a more accurate estimate of fentanyl exposure deaths; however, they do suggest that Zoorob's conclusions might not be robust to reasonable changes in the model. 



### Conclusion

Zoorob's estimates are more consistent with mortality statistics and can better explain the variation in overdose rates than any of the proposed extensions. The estimates in the extensions were either unrealistically low or impossibly high. This exercise of modifying the definition of fentanyl exposure exhibits the level of precision needed to use this measure and the specific way the state seizure data can be modified to understand the opioid epidemic and the recent fentanyl shock. While the impossibility of some of the estimates in this extension might appear to imply that his conclusions are not robust to reasonable changes in the model, this seems to point more broadly to the importance of acknowledging the role of illicit drug supply and drug environments in the fentanyl shock and opioid epidemic. Although Zoorob's conclusions are partly limited due to a lack of forensic testing and a lack of official fentanyl mortality data, they point to the significance of bringing variables like geography and a state's drug environment into the conversation about the opioid epidemic. Further exploration using Zoorob's methods can inform policymakers and help them implement addiction management programs and relief systems. Although drug overdose deaths decreased in 2018, fentanyl overdose deaths are still increasing. These increases underscore the urgent need to address the opioid epidemic in the United States and the continued work required to help those affected.
...
```{r ext2 table 1, results='asis', comment=NA, fig.pos="H", eval = FALSE}
############ FENTANYL EXPOSURE 2017
...
```{r ext2 table 2a/b, results='asis', comment=NA, fig.pos="H", fig.cap="Table 2 results for extension 2. Coefficient and death estimates are drastically lower than Zoorob's."}
#################################### TABLE 2
stargazer(minst2, minst1, type = 'latex', header = FALSE)
...
```{r ext3 table1, results='asis', comment=NA, fig.pos="H",eval=FALSE}
############ FENTANYL EXPOSURE 2017
...
```{r ext3 table 2, results='asis', comment=NA, fig.pos="H", fig.cap="Table 2 results for extension definition 3. Estimates are drastically higher than Zoorob's."}
#################################### TABLE 2
stargazer(minst2, minst1, type = 'latex', header = FALSE)
